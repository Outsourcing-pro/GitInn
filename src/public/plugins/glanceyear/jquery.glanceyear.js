...
//	console.log('jquery ',$)
    ...
    $.fn.glanceyear = function(massive, options) {
        ...
        var svgElement = createElementSvg('svg', {'width': 54*settings.cellSize+15, 'height': 7*settings.cellSize+15 } );
        ...
        //Weeks
        for (var i=0; i<54; i++) {
            var gElement = createElementSvg('g', {'transform': 'translate('+(settings.cellSize*i)+',0)'} );   
            ...
            for (var j=firstDate.getDay(); j<7 ; j++) {
                ...
                if ( rectDate.getMonth() != monthCount && i < 52 && j > 3 && daysLeft > 7) {
                    //new Month
                    var textMonth = createElementSvg('text', {'x': settings.cellSize*i, 'y':'-6', 'class':'month'} );
                ...
                if (dayCount>=0 || (settings.showToday && dayCount>=-1)) {
                    ...
                    var rectElement = createElementSvg('rect', {
                        'class': 'day',
                        'width': (settings.cellSize-2) + 'px',
                        'height': (settings.cellSize-2) + 'px',
                        'data-date': rectDate.getFullYear()+'-'+(rectDate.getMonth()+1)+'-'+rectDate.getDate(),
                        'y': settings.cellSize*j            
                    });
                    ...
                    rectElement.onmouseover = function() {
                        ...
                        if (tagCountData) {
                            if (tagCountData > 1 )
                                tagCount = $(this).attr('data-count')+' contributions';
                            else
                                tagCount = $(this).attr('data-count')+' contribution';
                        } else {
                            tagCount = 'No contributions';
                        }
        ...
        function fillData(massive) {
            ...
            $(settings.targetQuantity).text(massive.length + ' days, ' + scoreCount + ' contributions');
