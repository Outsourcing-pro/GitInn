<template>
	<div class="c_cnt c_contentBody c_flexRow cSection  c_relative">
		<div class="cntLeft">
			...
			<div class="c_flexRow ticketLHeader" style="position: relative;">
				<div class="c_relative">
					<div><img class="c_hd_img ticketLHeaderImg" :src="self.avatar_url" alt="" /></div>
					<div class="c_flexRow c_gray_txt ticketLHeaderDes c_relative" @click="showHeadMenu=true">{{self.username}} <img src="~img/src/多层级_下.svg" alt="" /></div>
			...
			<div class="">
				<div>
					<div class="cntLeftListItem topListItemLine c_bottom_line c_flexRow" :class="{active:activeTab==0}" @click="toggleTabs(0)">
						...
						<span style="flex: 1;">该用户仓库合并请求</span>
			...
			<div class="cntRightCnt">
				<div v-show="subTab==0" class="cntRightItem c_bottom_line dashed c_flexCol" v-for="item in sortTickets.open">
					<div class="c_flexRow" style="justify-content: flex-start;margin-bottom: 3px;">
						<div class="c_gray_label c_pointer" @click="$router.push({path: `/${item.repo.owner.username}/${item.repo.name}/pulls/${item.index}`})">{{item.repo.full_name}}#{{item.index}}</div>
			...
			loadTickets() {
				...
				axios.get('/users/' + this.self.username + '/repos').then(resp => {
					...
					axios.post('/issues/search', {
						...
						// 二次过滤，仓库的统计字段可能不准确的
                        this.repos = this.repos.filter(function(item) {
                            for (var i = 0; i < repoIDs.length; i++) {
                                if (repoIDs[i] == item.id) {
                                    return true;
                                }
                            }
                            return false;
