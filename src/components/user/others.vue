	...
	<div>
		<div class="selfPanel">
			<div class="c_cnt c_flexRow">
				...
				<div class="right">
					<ul class="c_flexRow">
						<li @click="$router.push({path:`${user.username}/followers`,query:{tab:1}})" class="c_pointer">
							<div class="rTitle">{{user.follower_count}}</div>
							<div class="c_gray_txt rDes">关注者</div>
						</li>
						<li @click="$router.push({path:`${user.username}/following`,query:{tab:2}})" class="c_pointer">
        ...
        <span class="glanceyear-quantity"></span>
				...
				<div class="c_gray_txt">
					<div class="historyItem c_bottom_line" v-for="item in ctrlList">
						...
						<!--<div class="c_flexRow">
							<img class="c_hd_img c_small_head_img" :src="item.act_avatar" alt="" />
							<span style="margin-left: 10px;">
								<span class="c_blue_color c_pointer">{{item.user_full_name}}</span>
							<span class="">评论了工单</span>
							<span class="c_blue_color c_pointer">proginn</span>
							</span>
							<span style="flex: 1;text-align: right;" class="c_gray_txt">{{$cmmDate(item.created,1)}}</span>
						</div>
						<div class="historyItemCmt">
			...
			<div class="storeList" v-show="tab==2">
				<div class="storeItem c_bottom_line c_flexRow" v-for="item in storeList">
					...
					<div class="sItemDes">
						<div class="c_blue_color c_pointer" @click="$router.push({path: `/${username}/${item.name}`})" style="font-size: 18px;">{{item.name}}
				...
				<!--<div class="storeItem c_bottom_line c_flexRow">
					<div class="c_bg_img sItemImg"></div>
					<div class="sItemDes">
						<div class="c_blue_color" style="font-size: 18px;">Glenn Barnett</div>
						<div>医疗美容平台-UI界面代码</div>
						<div class="c_gray_txt" style="font-size: 12px;margin-top: 3px;">三小时之前更新</div>
					</div>
					<div class="sItemInfo c_flexRow">
						<img src="" alt="" />
						<span>46</span>

						<img style="margin-left:40px;" src="" alt="" />
						<span>12</span>
					</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 加载用户信息
			loadUser() {
				axios.get('/users/' + this.username).then(resp => {
			...
			loadStoreList() {
				// 如果是其他人的主页，需要params
				axios.get('/users/' + this.username + '/repos',{  params:{
                            page: this.page,
			...
			loadCtrlList() {
				...
				//				axios.get('/users/'+username+'/contributes').then(resp=>{
				//					this.ctrlList=resp.data.commits
				//				}).catch(err=>Msg(err))
			},
            ...
            getCount() {
                //get readme data
                axios.get(`users/${this.username}/count/repos`,{params:{username: this.username}}).then(resp => {
			...
			initData(self) {
				...
				this.loadUser()
				this.loadStoreList(self)
				this.loadCtrlList(self)
				this.getCount();
			}
		},
		computed: {
